...
const MapView = forwardRef(function MapView(
  ...
  useEffect(() => {
    ...
    map.on('load', async () => {
      ...
      // ── 2. Transmission lines — lazy-loaded at zoom ≥ 3 to avoid 79 MB fetch on startup ──
      map.addSource('tx', { type: 'geojson', data: EMPTY_FC, tolerance: 0.5, buffer: 32 })
      map.addLayer({ id: 'tx-glow', type: 'line', source: 'tx', minzoom: 3,
        paint: {
          'line-color': ['get', 'c'],
      ...
      map.addLayer({ id: 'tx-core', type: 'line', source: 'tx', minzoom: 3,
        paint: {
          'line-color': ['get', 'c'],
          'line-width': ['interpolate', ['linear'], ['zoom'],
            3, ['*', ['get', 'w'], 1.0],
      ...
      // Fetch TX data lazily — fires once when user zooms to 3+
      let txLoaded = false
      const loadTx = async () => {
        if (txLoaded || dead) return
        txLoaded = true
      ...
      setMapReady(true)
        ...
        console.error('[MapView] load handler failed', e)
      }
    })
